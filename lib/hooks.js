...
export function useFile(repoId, fileId, branch = null) {
  ...
  const fetchFile = useCallback(async () => {
    ...
    setError(null);
    try {
      // Fetch file details and versions in parallel for better performance
      const [fileDetails, allVersionsData] = await Promise.all([
        api.files.getById(repoId, fileId, { branch }),
        // If branch is selected, get branch versions; otherwise get file versions
        branch 
          ? api.branches.getVersionHistory(repoId, branch)
          : api.files.getVersions(repoId, fileId)
      ]);
      
      setFile(fileDetails);

      // Process versions based on whether we're filtering by branch
      let fileVersions;
      if (branch) {
        console.log(`[useFile] Fetching branch versions for branch '${branch}'`);
        console.log(`[useFile] Branch versions response:`, allVersionsData);
        
        // Filter to only versions for this specific file
        fileVersions = allVersionsData
        ...
        // No branch selected, use all versions directly
        fileVersions = allVersionsData;
      }
