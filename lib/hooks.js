...
export function useFile(repoId, fileId, branch = null) {
  ...
  const fetchFile = useCallback(async () => {
    ...
    try {
      // Fetch file details
      const fileDetails = await api.files.getById(repoId, fileId, { branch });
      setFile(fileDetails);

      // For version history: if branch is selected, get ALL versions from that branch
      // Otherwise, get all versions of the file
      let fileVersions;
      if (branch) {
        console.log(`[useFile] Fetching branch versions for branch '${branch}'`);
        // Get all versions created in this branch using the branch versions endpoint
        const branchVersions = await api.branches.getVersionHistory(repoId, branch);
        console.log(`[useFile] Branch versions response:`, branchVersions);
        
        // Filter to only versions for this specific file
        fileVersions = branchVersions
          .filter(v => v.file_id === fileId)
          .map(v => ({
            id: v.version_id,
            version_number: v.version_number,
            created_at: v.created_at,
            commit_message: v.file_versions?.commit_message,
            file_size: v.file_versions?.file_size,
            mime_type: v.file_versions?.mime_type,
            content_hash: v.file_versions?.content_hash,
          }));
        
        console.log(`[useFile] Filtered ${fileVersions.length} versions for file ${fileId}`);
      } else {
        // No branch selected, get all versions of the file
        fileVersions = await api.files.getVersions(repoId, fileId);
      }
